...
export const showTransactionHash = (tranHash: string) => {
  return `${tranHash?.substring(0, 10)}${''.padStart(5, '*')}${tranHash?.substring(
    tranHash.length - 10,
    tranHash.length,
  )}`;
};
...
export const getToast = (
  ...
  if (typeof description === 'string') return { title, status, position: 'top-right', description, duration: 3000 };
